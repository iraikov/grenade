...
type FCNet = Network '[ FullyConnected 2 40, Tanh, FullyConnected 40 10, Relu, FullyConnected 10 1, Logit ]
                     '[ 'D1 2, 'D1 40, 'D1 40, 'D1 10, 'D1 10, 'D1 1, 'D1 1]

randomNet :: MonadRandom m => m FCNet
randomNet = randomNetwork

netTrain :: LearningParameters -> Int -> IO FCNet
netTrain rate n = do
...
netLoad :: FilePath -> IO FCNet
netLoad modelPath = do
  modelData <- B.readFile modelPath
  either fail return $ runGet (get :: Get FCNet) modelData
...
main = do
    ...
    net <- case load of
      ...
      Nothing -> netTrain rate examples
    netScore net
